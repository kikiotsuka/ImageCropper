...
def cleanup():
	...
	if os.path.isfile('tmpresize.jpg.jpg'): os.remove('tmpresize.jpg.jpg')
...
if resizemode:
	...
	print('Computing scaled image size. If image is large, operation may take a while')
	...
	os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (int((userscreenwidth - im2.size[0]) / 2), int((userscreenheight - im2.size[1]) / 2))
	windowSurfaceObj = pygame.display.set_mode((int(im2.size[0]), int(im2.size[1])))
	resizedimg = pygame.image.load('scaleoutput.jpg')
	if vertical:
		rectangle = Rect((0, int((windowSurfaceObj.get_size()[1] - userscreenheight * scalesize) / 2)), (int(userscreenwidth * scalesize), int(userscreenheight * scalesize)))
	else:
		rectangle = Rect((int((windowSurfaceObj.get_size()[0] - userscreenwidth * scalesize) / 2), 0), (int(userscreenwidth * scalesize), int(userscreenheight * scalesize)))
else: #simply load image for minimalistic mode
	os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (int((userscreenwidth - userscreenwidth * scalesize) / 2), int((userscreenheight - userscreenheight * scalesize) / 2))
	windowSurfaceObj = pygame.display.set_mode((int(userscreenwidth * scalesize), int(userscreenheight * scalesize)))
...
while isworking:
	...
	if resizemode:
		...
		if ask:
			if vertical:
				pygame.draw.rect(windowSurfaceObj, currentcolor, (0, 0, rectangle.width, rectangle.top), 0)
				pygame.draw.rect(windowSurfaceObj, currentcolor, (rectangle.left, rectangle.bottom, rectangle.width, windowSurfaceObj.get_size()[1] - rectangle.top), 0)
			else:
				pygame.draw.rect(windowSurfaceObj, currentcolor, (0, 0, rectangle.left, rectangle.height))
				pygame.draw.rect(windowSurfaceObj, currentcolor, (rectangle.right, rectangle.top, windowSurfaceObj.get_size()[0] - rectangle.right, rectangle.height), 0)
...
del im
print('Operation has been completed. The altered image has been saved as "' + str(imgname) + '"')
